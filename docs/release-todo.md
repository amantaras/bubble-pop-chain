...
- [x] Magnet: verify dial sweet spot, color-bubble shake, and lock timing on touch devices. Fixed a real bug: the ring's colour ramp was hand-tuned separately from `MAGNET_HALF` (the real strength falloff), so the dial still read yellow/orange for a wide stretch of travel where the mechanical pull strength had already floored at zero, misleading the player about how close their lock actually was. The ring gradient is now computed directly from `MAGNET_HALF` so it can never drift out of sync again. Sweet-spot randomisation, colour-bubble aim shake, and lock timing were already correct on inspection. Remaining: confirm feel on an actual touch device.
